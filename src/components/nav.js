...
const StyledLinks = styled.div`
  ...
  .resume-button {
    ...
    margin-left: 15px;
    display: none; // Resume button hidden for now
    font-size: var(--fz-xs);
  }
`;
              ...
              <ol>
                <TransitionGroup component={null}>
                  {isMounted &&
                    navLinks &&
                    navLinks.map(({ url, name }, i) => (
                      <CSSTransition key={i} nodeRef={linkRefs.current[i]} classNames={fadeDownClass} timeout={timeout}>
                        <li ref={linkRefs.current[i]} key={i} style={{ transitionDelay: `${isHome ? i * 100 : 0}ms` }}>
                          <Link to={url}>{name}</Link>
                        </li>
                      </CSSTransition>
                    ))}
